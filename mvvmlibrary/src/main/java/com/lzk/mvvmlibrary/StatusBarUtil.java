...
//        window.setStatusBarColor(Color.TRANSPARENT);
    ...
    /**
     * 增加View的高度增加的值为状态栏高度.一般是在沉浸式全屏给ToolBar用的
     */
    public static int setHeight(Context context, View view) {
        ViewGroup.LayoutParams lp = view.getLayoutParams();
        if (Build.VERSION.SDK_INT >= MIN_API) {
            return lp.height += getStatusBarHeight(context);//增高
        }
        return lp.height;
    }
